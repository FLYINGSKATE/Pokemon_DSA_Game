...
export default function RegionPage() {
  ...
  const regionRaw: string = Array.isArray(regionParam) ? regionParam[0] : (regionParam || "");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 via-blue-50 to-pink-50 p-6 flex flex-col items-center">
      <h1 className="text-3xl font-extrabold text-indigo-700 mb-4">{regionDisplay} Region</h1>
      <p className="text-lg text-gray-700 mb-8">Welcome to the {regionDisplay} region! Catch and train algorithms from this region below.</p>
      <div className="flex flex-wrap gap-8 justify-center">
        {regionPokedex.map((poke: PokeAlgo) => (
          <div key={poke.name} className="bg-white/90 rounded-xl shadow-lg p-6 max-w-md w-full border-4 border-yellow-300 flex flex-col gap-4">
            ...
            <details className="mt-2">
              ...
              <ul className="mt-2 flex flex-col gap-2">
                {poke.leetcode.map((prob: { title: string; url: string; sample: string }) => (
